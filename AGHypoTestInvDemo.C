...
void AGHypoTestInvDemo(const char * fileName =0,
                                      ...
                                      int ntoys, bool useCls ) 
{
   ...
   if (!data) { 
      Error("AGHypoTestDemo","Not existing data %s",dataName);
      return 0;
   }
   else 
   ...
   if (!sbModel) {
      Error("AGHypoTestDemo","Not existing ModelConfig %s",modelSBName);
      return 0;
   }
   // check the model 
   if (!sbModel->GetPdf()) { 
      Error("AGHypoTestDemo","Model %s has no pdf ",modelSBName);
      return 0;
   }
   if (!sbModel->GetParametersOfInterest()) {
      Error("AGHypoTestDemo","Model %s has no poi ",modelSBName);
      return 0;
   }
   if (!sbModel->GetParametersOfInterest()) {
      Error("AGHypoTestInvDemo","Model %s has no poi ",modelSBName);
      return 0;
   }
   if (!sbModel->GetSnapshot() ) { 
      Info("AGHypoTestInvDemo","Model %s has no snapshot  - make one using model poi",modelSBName);
      sbModel->SetSnapshot( *sbModel->GetParametersOfInterest() );
   }


   if (!bModel || bModel == sbModel) {
      Info("AGHypoTestInvDemo","The background model %s does not exist",modelBName);
      Info("AGHypoTestInvDemo","Copy it from ModelConfig %s and set POI to zero",modelSBName);
      ...
      if (!bModel->GetSnapshot() ) { 
         Info("AGHypoTestInvDemo","Model %s has no snapshot  - make one using model poi and 0 values ",modelBName);
         RooRealVar * var = dynamic_cast<RooRealVar*>(bModel->GetParametersOfInterest()->first());
         if (var) { 
            double oldval = var->getVal();
            var->setVal(0);
            ...
            Error("AGHypoTestInvDemo","Model %s has no valid poi",modelBName);
            return 0;
         }         
      }
   }
   ...
   if (type == 1) { 
      ...
      if (bModel->GetPriorPdf() && sbModel->GetPriorPdf() ) {
         ...
         if (bModel->GetNuisanceParameters() || sbModel->GetNuisanceParameters() ) {
            Error("AGHypoTestInvDemo","Cannnot run Hybrid calculator because no prior on the nuisance parameter is specified");
            return 0;
         }
      }
   } 
...
void ReadResult(const char * fileName, const char * resultName="", bool useCLs=true) { 
   ...
   AGHypoTestInvDemo(fileName, resultName,"","","",0,0,useCLs);
}
