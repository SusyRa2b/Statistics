//
//  dmc_drawplots_t1tttt( infile, savePdf, recycleCanvas, histnamepat )
//
  ...
  //----------
    ...
    void dmc_drawplots_t1tttt( const char* infile = "rootfiles/dmc_plots_t1tttt_njetsge5_all.root", 
       ...
       if ( withData ) {
          ...
          nComps = 6 ;
          sprintf( compname[0], "diboson" ) ;   kfactor[0] = 1.00 ;
          sprintf( compname[1], "znn" ) ;       kfactor[1] = 1.00 ;
          sprintf( compname[2], "qcd" ) ;       kfactor[2] = 1.80 ; //**********************
          sprintf( compname[3], "singlet" ) ;   kfactor[3] = 1.00 ;
          sprintf( compname[4], "wjets" ) ;     kfactor[4] = 0.90 ; //**********************
          sprintf( compname[5], "ttbar" ) ;     kfactor[5] = 0.90 ; //**********************
       }
  ...
  //--------------------------------------------------------
   ...
   void drawSetLHB( const char* hname_base ) {
      ...
      if ( withData ) {
         ...
         hdata = (TH1F*) gDirectory->FindObject( hname ) ;
         if ( hdata == 0x0 ) {
   ...
   // hmcsum -> Reset() ;
      ...
      int startComp ;
      if ( withData ) { startComp = 1 ; } else { startComp = 0 ; }  // data, if being plotted, is comp 0.

      TH1F* hmcsum(0x0) ;

      for ( int ci=startComp; ci<nComps; ci++ ) {
         sprintf( hname, "%s_%s_flat", hname_base, compname[ci] ) ;
         TH1F* hmc = (TH1F*) gDirectory->FindObject( hname ) ;
         if ( ci==startComp ) {
            char sumhname[1000] ;
            sprintf( sumhname, "%s_mcsum", hname_base ) ;
            hmcsum = (TH1F*) hmc->Clone( sumhname ) ;
            hmcsum->Reset() ;
         }
      ...
      for ( int ci=nComps-1; ci>=startComp; ci-- ) {
         sprintf( hname, "%s_%s_flat", hname_base, compname[ci] ) ;
         TH1F* hmc = (TH1F*) gDirectory->FindObject( hname ) ;
      ...
      hmcsum->Draw("esame") ;
      if ( withData) hdata->DrawCopy("same") ;
      legend->Draw() ;
      toppad->Update() ;
      toppad->Draw() ;
      TString htitle( hfirst->GetTitle() ) ;
      char killstring[100] ;
      sprintf( killstring, ", %s", compname[0] ) ;
      htitle.ReplaceAll( killstring, "" ) ;
      TText* title = new TText() ;
      title->SetTextSize(0.040) ;
      title->DrawTextNDC( 0.05, 0.95, htitle ) ;
      ...
      if ( savePdf ) {
         ...
         dataset.ReplaceAll("rootfiles/dmc_plots_t1tttt_","") ;
   ...
   // hmcsum -> Reset() ;
      ...
      int startComp ;
      if ( withData ) { startComp = 1; } else { startComp = 0; }
      ...
      htitle.ReplaceAll( killstring, "" ) ;
      title->DrawTextNDC( 0.05, 0.95, htitle ) ;
      if ( isMetPlot ) { drawMetBinEdges( hfirst ) ; }
      if ( isHtPlot )  { drawHtBinEdges( hfirst ) ; }
