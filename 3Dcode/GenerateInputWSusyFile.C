...
void GenerateInputWSusyFile() {
  ...
  TChain* t1bbbbTree = new TChain("tree") ;
  nAdded = t1bbbbTree->Add("files5fb/T1bbbb.root") ;
  if ( nAdded <= 0 ) {
...
//float dummyFloat = 9.999;
  ...
  ofstream inFile;
  char outfile[10000] ;
  sprintf( outfile, "InputWSusy-met%d-ht%d.dat", nBinsMET, nBinsHT ) ;
  inFile.open( outfile );
