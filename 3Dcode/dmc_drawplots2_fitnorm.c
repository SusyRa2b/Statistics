...
#include <iostream>
  ...
  // prototypes
   ...
   void drawSet( const char* hname_base, const char* xtitle, const char* norm_key ) ;
   void drawSetLHB( const char* hname_base, const char* norm_key ) ;
  ...
  //----------
    ...
    void dmc_drawplots2_fitnorm( const char* infile = "rootfiles/dmc_plots2_all.root", 
     ...
     //---------
       ...
       sprintf( histnamelist[hind], "h_met_sl_all" ) ; sprintf( histxtitlelist[hind], "MET (GeV)" ) ; sprintf( normkey[hind], "sl_wt" ) ; hind++ ;
       sprintf( histnamelist[hind], "h_met_sl_nb0" ) ; sprintf( histxtitlelist[hind], "MET (GeV)" ) ; sprintf( normkey[hind], "none" ) ; hind++ ;
       sprintf( histnamelist[hind], "h_met_sl_nb1" ) ; sprintf( histxtitlelist[hind], "MET (GeV)" ) ; sprintf( normkey[hind], "sl_wt_1b" ) ; hind++ ;
       sprintf( histnamelist[hind], "h_met_sl_nb2" ) ; sprintf( histxtitlelist[hind], "MET (GeV)" ) ; sprintf( normkey[hind], "sl_wt_2b" ) ; hind++ ;
       sprintf( histnamelist[hind], "h_met_sl_nb3" ) ; sprintf( histxtitlelist[hind], "MET (GeV)" ) ; sprintf( normkey[hind], "sl_wt_3b" ) ; hind++ ;
     ...
     //--- log y scale.
       ...
       for ( int hi=0; hi<nhistlist; hi++ ) {
          if ( strlen(histnamepat) > 0 ) {
             ...
             if ( str.Contains( histnamepat ) ) {
                drawSet( histnamelist[hi], histxtitlelist[hi], normkey[hi] ) ;
             }
          } else {
             drawSet( histnamelist[hi], histxtitlelist[hi], normkey[hi] ) ;
          }
       ...
       for ( int hi=0; hi<nhistlist_lhb; hi++ ) {
          if ( strlen(histnamepat) > 0 ) {
             ...
             if ( str.Contains( histnamepat ) ) {
                drawSetLHB( histnamelist_lhb[hi], normkey[hi] ) ;
             }
          } else {
             drawSetLHB( histnamelist_lhb[hi], normkey[hi] ) ;
          }
      ...
      for ( int ci=1; ci<nComps; ci++ ) {
         ...
         if ( !kfactorAlreadyApplied ) {
            if ( strcmp( norm_key, "none" ) != 0 ) {
               ...
               double integral = hmc->Integral() ;
               char normtag[1000] ;
               sprintf( normtag, "%s_%s", compname[ci], norm_key ) ;
               float fit_norm(0.) ;
