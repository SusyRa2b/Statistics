...
SamplingDistribution* ToyMCSampler::GetSamplingDistribution(RooArgSet& paramPointIn) {
   ...
   cout << "\n\n ToyMCSampler::GetSamplingDistribution: just getting started.\n\n" << flush ;
...
SamplingDistribution* ToyMCSampler::GetSamplingDistributionSingleWorker(RooArgSet& paramPointIn) {
   ...
   cout << "\n\n ToyMCSampler::GetSamplingDistributionSingleWorker: just getting started.\n\n" << flush ;
   ...
   cout << " ToyMCSampler::GetSamplingDistributionSingleWorker: paramPoint\n" << flush ;
   paramPoint->Print() ;
   TIterator* parIter = paramPoint->createIterator() ;
   double mu_susy_sig_val(-1.) ;
   while ( RooRealVar* par = (RooRealVar*) parIter->Next() ) {
      cout << par->GetName() << " : " << par->getVal() << endl ;
      if ( strcmp( par->GetName(), "mu_susy_sig" )== 0 ) { mu_susy_sig_val = par->getVal() ; }
   }
   cout << "\n\n ToyMCSampler::GetSamplingDistributionSingleWorker: mu_susy_sig value is " << mu_susy_sig_val << "\n\n" << flush ;
  ...
  //=== DEBUG code =================================================================
   ...
   for (Int_t i = 0; i < fMaxToys; ++i) {
      ...
      *allVars = *paramPoint;
      ...
      if(!fImportanceDensity) {
         cout << " ToyMCSampler::GetSamplingDistributionSingleWorker: calling GenerateToyData\n" << flush ;
         ...
         cout << " ToyMCSampler::GetSamplingDistributionSingleWorker: toy data.\n" << flush ;
         toydata->Print() ;
         toydata->printMultiline(cout, 1, kTRUE, "") ;
         ...
         cout << " ToyMCSampler::GetSamplingDistributionSingleWorker: calling GenerateToyDataImportanceSampling\n" << flush ;
         // generate toy data for this parameter point
