// @(#)root/roostats:$Id: AsymptoticCalculatorNestedSimPdf.cxx 47012 2012-11-02 16:06:59Z moneta $
...
ClassImp(RooStats::AsymptoticCalculatorNestedSimPdf);

int AsymptoticCalculatorNestedSimPdf::fgPrintLevel = 1;

void AsymptoticCalculatorNestedSimPdf::SetPrintLevel(int level) { 
   // set print level (static function)
   // 0 minimal, 1 normal,  2 debug
   fgPrintLevel = level;
}

AsymptoticCalculatorNestedSimPdf::AsymptoticCalculatorNestedSimPdf(
   RooAbsData &data,
...
RooAbsData * AsymptoticCalculatorNestedSimPdf::MakeAsimovData(RooAbsData & realData, const ModelConfig & model, const  RooArgSet & paramValues, RooArgSet & asimovGlobObs, const RooArgSet * genPoiValues )  {
...
RooAbsData * AsymptoticCalculatorNestedSimPdf::GenerateAsimovData(const RooAbsPdf & pdf, const RooArgSet & observables  )  { 
  ...
  //RooDataSet* simData=NULL;
   ...
   if(prodPdf)
	 ...
	 {
	   const RooAbsPdf *pdfi = dynamic_cast<const RooAbsPdf*>(list.at(i));
	   if(pdfi->dependsOn(observables)) strippedPdfSet.add(*pdfi);
	 }
     }
   RooProdPdf observableProdPdf("observableProdPdf","observableProdPdf",strippedPdfSet);
   if(prodPdf)
     {
       if(strippedPdfSet.getSize() == 1) observablePdf = dynamic_cast<const RooAbsPdf*>(strippedPdfSet.at(0));
       else observablePdf = &observableProdPdf;
     }
   else observablePdf = &pdf;
   observablePdf->Print();

   const RooSimultaneous* simPdf = dynamic_cast<const RooSimultaneous*>(observablePdf);
   if (!simPdf) { 
      // generate data for non sim pdf
      return GenerateAsimovDataSinglePdf( *observablePdf, observables, *weightVar, 0);
   }
   cout <<"found simultaneous pdf" << endl;
...
//______________________________________________________________________________
RooAbsData * AsymptoticCalculatorNestedSimPdf::MakeAsimovData(const ModelConfig & model, const  RooArgSet & allParamValues, RooArgSet & asimovGlobObs)  {
   // static function to the an Asimov data set
   ...
   if (model.GetGlobalObservables() && model.GetGlobalObservables()->getSize() > 0) {
      ...
      if (nuis.getSize() == 0) { 
            oocoutW((TObject*)0,Generation) << "AsymptoticCalculatorNestedSimPdf::MakeAsimovData: model does not have nuisance parameters but has global observables"
      ...
      std::auto_ptr<RooAbsPdf> nuispdf( extractNuisancePdf(model,"TempNuisPdf") );
      // unfold the nuisance pdf 
      RooProdPdf *prod = dynamic_cast<RooProdPdf *>(nuispdf.get());
      if (prod == 0) { 
         oocoutF((TObject*)0,Generation) << "AsymptoticCalculatorNestedSimPdf::MakeAsimovData: the nuisance pdf is not a RooProdPdf!" << std::endl;
      }
      ...
      for (RooAbsArg *a = (RooAbsArg *) iter->Next(); a != 0; a = (RooAbsArg *) iter->Next()) {
         ...
         if (cgobs->getSize() != 1) {
            oocoutE((TObject*)0,Generation) << "AsymptoticCalculatorNestedSimPdf::MakeAsimovData: constraint term  " <<  cterm->GetName() 
         ...
         if (cpars->getSize() != 1) {
            oocoutE((TObject*)0,Generation) << "AsymptoticCalculatorNestedSimPdf::MakeAsimovData:constraint term " 
         ...
         if ( cClass != RooGaussian::Class() && cClass != RooPoisson::Class() &&
            ...
            oocoutW((TObject*)0,Generation) << "AsymptoticCalculatorNestedSimPdf::MakeAsimovData:constraint term " 
         ...
         if (!foundServer) {  
            oocoutE((TObject*)0,Generation) << "AsymptoticCalculatorNestedSimPdf::MakeAsimovData - can't find nuisance for constraint term - global observales will not be set to Asimov value " << cterm->GetName() << std::endl;
...
void AsymptoticCalculatorNestedSimPdf::factorizePdf(const RooArgSet &observables, RooAbsPdf &pdf, RooArgList &obsTerms, RooArgList &constraints) {
...
RooAbsPdf * AsymptoticCalculatorNestedSimPdf::extractNuisancePdf(RooAbsPdf &pdf, const RooArgSet &observables, const char *name) { 
  RooArgList obsTerms, constraints;
  factorizePdf(observables, pdf, obsTerms, constraints);
  return new RooProdPdf(name,"", constraints);
}

RooAbsPdf * AsymptoticCalculatorNestedSimPdf::extractNuisancePdf(const RooStats::ModelConfig &model, const char *name) { 
  return extractNuisancePdf(*model.GetPdf(), *model.GetObservables(), name);
}

//_________________________________________________________________
Double_t AsymptoticCalculatorNestedSimPdf::EvaluateNLL(RooAbsPdf & pdf, RooAbsData& data,   const RooArgSet *poiSet) {
    ...
    if (skipFit) 
       ...
       if (verbose > 0 )
          std::cout << "AsymptoticCalculatorNestedSimPdf::EvaluateNLL  ........ using " << minimizer << " / " << algorithm 
    ...
    double muTest = 0; 
    if (verbose > 0) { 
       std::cout << "AsymptoticCalculatorNestedSimPdf::EvaluateNLL -  value = " << val;
       if (poiSet) { 
...
HypoTestResult* AsymptoticCalculatorNestedSimPdf::GetHypoTest() const {
   ...
   if (!fAsimovData) { 
       oocoutE((TObject*)0,InputArguments) << "AsymptoticCalculatorNestedSimPdf::GetHypoTest - Asimov data set has not been generated - return NULL result " << endl;
       return 0;
   }
   ...
   if (poiTest.getSize() > 1)  { 
      oocoutW((TObject*)0,InputArguments) << "AsymptoticCalculatorNestedSimPdf::GetHypoTest: snapshot has more than one POI - assume as POI first parameter " << std::endl;         
   }
   ...
   *allParams = fBestFitParams;
   ...
   if (verbose> 0) {
      std::cout << std::endl;
      oocoutI((TObject*)0,Eval) << "AsymptoticCalculatorNestedSimPdf::GetHypoTest: - perform  an hypothesis test for  POI ( " << muTest->GetName() << " ) = " << muTest->getVal() << std::endl;
      oocoutP((TObject*)0,Eval) << "AsymptoticCalculatorNestedSimPdf::GetHypoTest -  Find  best conditional NLL on OBSERVED data set ..... " << std::endl;
   }
   ...
   if (verbose > 0) oocoutP((TObject*)0,Eval) << "AsymptoticCalculatorNestedSimPdf::GetHypoTest -- Find  best conditional NLL on ASIMOV data set .... " << std::endl;
...
double AsymptoticCalculatorNestedSimPdf::GetExpectedPValues(double pnull, double palt, double nsigma, bool useCls, bool oneSided ) { 
...
void AsymptoticCalculatorNestedSimPdf::FillBins(const RooAbsPdf & pdf, const RooArgList &obs, RooAbsData & data, int &index,  double &binVolume, int &ibin) { 
...
bool AsymptoticCalculatorNestedSimPdf::SetObsToExpected(RooProdPdf &prod, const RooArgSet &obs) 
{
...
bool AsymptoticCalculatorNestedSimPdf::SetObsToExpected(RooAbsPdf &pdf, const RooArgSet &obs) 
{
   ...
   for (RooAbsArg *a = (RooAbsArg *) iter->Next(); a != 0; a = (RooAbsArg *) iter->Next()) {
      if (obs.contains(*a)) {
         if (myobs != 0) { 
            oocoutF((TObject*)0,Generation) << "AsymptoticCalculatorNestedSimPdf::SetObsExpected( " << pdfName << " ) : Has two observables ?? " << endl;
            return false;
         }
         myobs = dynamic_cast<RooRealVar *>(a);
         if (myobs == 0) { 
            oocoutF((TObject*)0,Generation) << "AsymptoticCalculatorNestedSimPdf::SetObsExpected( " << pdfName << " ) : Observable is not a RooRealVar??" << endl;
            return false; 
         }
      } else {
         if (!a->isConstant() ) {
            if (myexp != 0) { 
               oocoutE((TObject*)0,Generation) << "AsymptoticCalculatorNestedSimPdf::SetObsExpected( " << pdfName << " ) : Has two non-const arguments  " << endl;
               return false;
            }
            myexp = dynamic_cast<RooAbsReal *>(a);
            if (myexp == 0) { 
               oocoutF((TObject*)0,Generation) << "AsymptoticCalculatorNestedSimPdf::SetObsExpected( " << pdfName << " ) : Expected is not a RooAbsReal??" << endl;
               return false; 
            }
         }
      }
   ...
   if (myobs == 0)  { 
      oocoutF((TObject*)0,Generation) << "AsymptoticCalculatorNestedSimPdf::SetObsExpected( " << pdfName << " ) : No observable?" << endl;
      return false;
   }
   if (myexp == 0) {    
      oocoutF((TObject*)0,Generation) << "AsymptoticCalculatorNestedSimPdf::SetObsExpected( " << pdfName << " ) : No observable?" << endl;
...
RooAbsData * AsymptoticCalculatorNestedSimPdf::GenerateCountingAsimovData(RooAbsPdf & pdf, const RooArgSet & observables,  const RooRealVar & , RooCategory * channelCat) { 
...
RooAbsData * AsymptoticCalculatorNestedSimPdf::GenerateAsimovDataSinglePdf(const RooAbsPdf & pdf, const RooArgSet & allobs,  const RooRealVar & weightVar, RooCategory * channelCat) { 
