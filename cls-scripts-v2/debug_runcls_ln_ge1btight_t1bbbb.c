   ...
   void debug_runcls_ln_ge1btight_t1bbbb( int m0 = 560, int m12 = 300, double t1bbbbXsec = 1.0, int iter = 1, const char* rootdir = "output-root-files" ) {

      gROOT->LoadMacro("ra2bRoostatsClass4ln.c+") ;


      printf("\n\n root output directory is %s\n\n", rootdir ) ;
      ...
      char outfname[10000] ;
   // sprintf( outfname, "debug-output-files-ge1btight-t1bbbb-iter%d/clstest_ge1btight_t1bbbb_%d_%d.root", iter, m0, m12 ) ;
      sprintf( outfname, "%s/clstest_ge1btight_t1bbbb_%d_%d_iter%d.root", rootdir, m0, m12, iter ) ;
      printf("\n\n Opening output root file: %s\n", outfname ) ;
