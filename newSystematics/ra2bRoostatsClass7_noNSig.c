...
#include "ra2bRoostatsClass7_noNSig.h"
 ...
 ///  rv_Nlsb_0b_ldp = new RooRealVar( "Nlsb_0b_ldp" , "Nlsb_0b_ldp" , 0.0, 1000000. ) ;
  ...
  /// rv_Nlsb_0b_ldp -> setVal( Nhtonlytrig_lsb_0b_ldp ) ;
    ...
    //____ Counts in SB, SL  ______________________
      ...
      allPoissonNuisances.add(*rv_mu_ttwj_sb_sl);
 ...
 //   rv_mu_qcd_lsb_0b_ldp  -> setVal( initialval_qcd_lsb_0b_ldp ) ;  //-- this is a starting value only.
   ...
   //++++++++++
    ...
    //-------  add trigger efficiencies as nuisance parameters with Beta Funtions
      ...
      betaModeTransform(eps_sb_sl_mean-epsSF_sb_sl_errm+0.5*(epsSF_sb_sl_errm+epsSF_sb_sl_errp),epsSF_sb_sl_errm+epsSF_sb_sl_errp,alpha,beta);
      RooRealVar rrv_epsSF_sb_sl ("epsSF_sb_sl","epsSF_sb_sl",eps_sb_sl_mean-epsSF_sb_sl_errm+0.5*(epsSF_sb_sl_errm+epsSF_sb_sl_errp),0,1);
      RooRealVar epsSF_sb_sl_alpha ("epsSF_sb_sl_alpha", "epsSF_sb_sl_alpha", alpha);
      RooRealVar epsSF_sb_sl_beta ("epsSF_sb_sl_beta", "epsSF_sb_sl_beta", beta);
      RooBetaPdf pdf_epsSF_sb_sl ("pdf_epsSF_sb_sl" , "pdf_epsSF_sb_sl", rrv_epsSF_sb_sl, epsSF_sb_sl_alpha, epsSF_sb_sl_beta);
      epsSF_sb_sl_alpha.setConstant();
      epsSF_sb_sl_beta.setConstant();
      allNonPoissonNuisances.add (rrv_epsSF_sb_sl);
      if(constantNonPoisson) rrv_epsSF_sb_sl.setConstant();
      betaNuisancePdfs.add (pdf_epsSF_sb_sl);
    ...
    //--------------------------------------
      ...
      double sigma_max = max(sigma_eff_sf_sig,
			     max(sigma_eff_sf_sb,
				 max(sigma_eff_sf_sig_sl,
				     max(sigma_eff_sf_sb_sl,
					 max(sigma_eff_sf_sig_ldp,
					     sigma_eff_sf_sb_ldp)))));

      sigma_avg =(sigma_eff_sf_sig + sigma_eff_sf_sb + sigma_eff_sf_sig_sl + sigma_eff_sf_sb_sl + sigma_eff_sf_sig_ldp + sigma_eff_sf_sb_ldp)/6.;
      mu_avg = 1.;
	...
	alphaRoot.Solve();
      ...
      if(sigma_eff_sf_sig_sl > 0){
	alphaFinding.setDesiredSigma(sigma_eff_sf_sig_sl);
	alphaRoot.SetFunction(WrappedAlphaFindingFunction, 1e-6, 5*sigma_max/sigma_avg);
	alphaRoot.Solve();
      }
      RooRealVar eff_sf_sig_sl_alpha("eff_sf_sig_sl_alpha","eff_sf_sig_sl_alpha", (sigma_eff_sf_sig_sl > 0) ? alphaRoot.Root() : 0 );
      eff_sf_sig_sl_alpha.setConstant();

      if(sigma_eff_sf_sb_sl > 0){
	alphaFinding.setDesiredSigma(sigma_eff_sf_sb_sl);
	alphaRoot.SetFunction(WrappedAlphaFindingFunction, 1e-6, 5*sigma_max/sigma_avg);
	alphaRoot.Solve();
      }
      RooRealVar eff_sf_sb_sl_alpha("eff_sf_sb_sl_alpha","eff_sf_sb_sl_alpha", (sigma_eff_sf_sb_sl > 0) ? alphaRoot.Root() : 0 );
      eff_sf_sb_sl_alpha.setConstant();
	...
	alphaRoot.Solve();
      ...
      cout << "eff_sf section -- eff_sf_sig_sl  has alpha of: " << eff_sf_sig_sl_alpha .getVal() << endl;
      cout << "eff_sf section -- eff_sf_sb_sl   has alpha of: " << eff_sf_sb_sl_alpha  .getVal() << endl;
      ...
      RooFormulaVar fv_eff_sf_sig_sl  ("eff_sf_sig_sl"  , "pow(@0,@1)" , RooArgList( rrv_eff_sf , eff_sf_sig_sl_alpha ));
      RooFormulaVar fv_eff_sf_sb_sl   ("eff_sf_sb_sl"   , "pow(@0,@1)" , RooArgList( rrv_eff_sf , eff_sf_sb_sl_alpha  ));
      ...
      RooRealVar eff_sf_sig_sl_sigma  ("eff_sf_sig_sl_sigma" , "eff_sf_sig_sl_sigma"  , sigma_eff_sf_sig_sl  );
      RooRealVar eff_sf_sb_sl_sigma   ("eff_sf_sb_sl_sigma"  , "eff_sf_sb_sl_sigma"   , sigma_eff_sf_sb_sl   );
...
/*       allNuisancePdfs.add (pdf_xxx); */
    ...
    //-- ttwj
      ...
      if ( useSigTtwjVar ) {
         rfv_mu_ttwj_sb = new RooFormulaVar("mu_ttwj_sb",
					     "mu_ttwj_sig * (1.0/sf_ttwj_sig) * (mu_ttwj_sb_sl/mu_ttwj_sig_sl) * (1.0/epsSF_sb) * (epsSF_sig_sl/epsSF_sb_sl)",
					     RooArgSet( *rv_mu_ttwj_sig, rrv_sf_ttwj_sig, *rv_mu_ttwj_sb_sl, *rv_mu_ttwj_sig_sl, 
							rrv_epsSF_sb, rrv_epsSF_sig_sl, rrv_epsSF_sb_sl) ) ;
         rv_mu_ttwj_sb = rfv_mu_ttwj_sb ;
      } else {
         rfv_mu_ttwj_sig = new RooFormulaVar("mu_ttwj_sig",
					      "mu_ttwj_sb * sf_ttwj_sig * (mu_ttwj_sig_sl/mu_ttwj_sb_sl) * (1.0/epsSF_sb) * (epsSF_sb_sl/epsSF_sig_sl)",
					      RooArgSet( *rv_mu_ttwj_sb, rrv_sf_ttwj_sig, *rv_mu_ttwj_sig_sl, *rv_mu_ttwj_sb_sl, 
							 rrv_epsSF_sb, rrv_epsSF_sb_sl, rrv_epsSF_sig_sl ) ) ;
   ...
   //-------------------------------
      ...
      if ( useLdpVars ) {
         ...
         rfv_mu_qcd_sb = new RooFormulaVar("mu_qcd_sb",
					    "mu_qcd_sb_ldp * sf_qcd_sb * Rlsb_passfail * (1.0/epsSF_sb_ldp)",
					    RooArgSet( *rv_mu_qcd_sb_ldp, rrv_sf_qcd_sb, rrv_Rlsb_passfail, rrv_epsSF_sb_ldp ) ) ;
         rv_mu_qcd_sb = rfv_mu_qcd_sb ;

      } else {

         rfv_mu_qcd_sig_ldp = new RooFormulaVar("mu_qcd_sig_ldp",
						 "mu_qcd_sig * (1.0/sf_qcd_sig) * ( 1.0 / Rlsb_passfail ) * (1.0/epsSF_sig)",
						 RooArgSet( *rv_mu_qcd_sig, rrv_sf_qcd_sig, rrv_Rlsb_passfail, rrv_epsSF_sig ) ) ;
         rv_mu_qcd_sig_ldp = rfv_mu_qcd_sig_ldp ;

         rfv_mu_qcd_sb_ldp = new RooFormulaVar("mu_qcd_sb_ldp",
						"mu_qcd_sb * (1.0/sf_qcd_sb) * ( 1.0 / Rlsb_passfail ) * (1.0/epsSF_sb_ldp)",
						RooArgSet( *rv_mu_qcd_sb, rrv_sf_qcd_sb, rrv_Rlsb_passfail, rrv_epsSF_sb_ldp ) ) ;
         rv_mu_qcd_sb_ldp = rfv_mu_qcd_sb_ldp ;
   ...
   //-------------------------------
    ...
    //-- SUSY
      ...
      rv_mu_susy_sb = new RooFormulaVar("mu_susy_sb",
                                        "mu_susymc_sb * (mu_susy_sig/mu_susymc_sig) * (1.0/epsSF_sb)",
					 RooArgSet( *rv_mu_susymc_sb, *rv_mu_susy_sig, *rv_mu_susymc_sig, rrv_epsSF_sb ) ) ;

      rv_mu_susy_sig_sl = new RooFormulaVar("mu_susy_sig_sl",
                                        "mu_susymc_sig_sl * (mu_susy_sig/mu_susymc_sig) * (1.0/epsSF_sig_sl)",
					     RooArgSet( *rv_mu_susymc_sig_sl, *rv_mu_susy_sig, *rv_mu_susymc_sig, rrv_epsSF_sig_sl ) ) ;

      rv_mu_susy_sb_sl = new RooFormulaVar("mu_susy_sb_sl",
					    "mu_susymc_sb_sl * (mu_susy_sig/mu_susymc_sig) * (1.0/epsSF_sb_sl)",
					    RooArgSet( *rv_mu_susymc_sb_sl, *rv_mu_susy_sig, *rv_mu_susymc_sig, rrv_epsSF_sb_sl ) ) ;
   ...
   //---------------------------------
    ...
    //+++++++++++++ Expected counts for observables in terms of parameters ++++++++++++++++++
      ...
      rv_n_sig         = new RooFormulaVar("n_sig",
					    ...
					    RooArgSet( *rv_mu_ttwj_sig, *rv_mu_qcd_sig, *rv_mu_znn_sig, fv_eff_sf_sig, *rv_mu_ewo_sig, *rv_mu_susy_sig ) ) ;
      
      rv_n_sb          = new RooFormulaVar("n_sb",
					    "mu_ttwj_sb  + mu_qcd_sb  + mu_znn_sb  + eff_sf_sb*( mu_ewo_sb  + mu_susy_sb )",
					    RooArgSet( *rv_mu_ttwj_sb , *rv_mu_qcd_sb , *rv_mu_znn_sb , fv_eff_sf_sb, *rv_mu_ewo_sb , *rv_mu_susy_sb  ) ) ;
      ...
      rv_n_sig_sl      = new RooFormulaVar("n_sig_sl",
					    ...
					    RooArgSet( *rv_mu_ttwj_sig_sl, fv_eff_sf_sig_sl, *rv_mu_susy_sig_sl ) ) ;

      rv_n_sb_sl       = new RooFormulaVar("n_sb_sl",
					    "mu_ttwj_sb_sl + eff_sf_sb_sl*mu_susy_sb_sl",
					    RooArgSet( *rv_mu_ttwj_sb_sl, fv_eff_sf_sb_sl, *rv_mu_susy_sb_sl ) ) ;
