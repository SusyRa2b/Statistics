...
#ifndef ROOSTATS_ProfileLikelihoodTestStat_New
#define ROOSTATS_ProfileLikelihoodTestStat_New

//_________________________________________________
/*
BEGIN_HTML
<p>
ProfileLikelihoodTestStat_New is an implementation of the TestStatistic interface that calculates the profile
...
namespace RooStats {
  ...
  class ProfileLikelihoodTestStat_New : public TestStatistic{

   public:
     ProfileLikelihoodTestStat_New() {
	...
	//fDoProfllCheck = false;
	fDebuggingUncondData = NULL;
	fDebuggingCondData = NULL;
	fDebuggingGenData = NULL;
	fFluctuatedNuisanceParameters = NULL;
     }
     ProfileLikelihoodTestStat_New(RooAbsPdf& pdf) {
       fPdf = &pdf;
       fProfile = 0;
       fNll = 0;
       fCachedBestFitParams = 0;
     ...
     virtual ~ProfileLikelihoodTestStat_New() {
       //       delete fRand;
       //       delete fTestStatistic;
       if(fProfile) delete fProfile;
       if(fNll) delete fNll;
       if(fCachedBestFitParams) delete fCachedBestFitParams;
       if(fFluctuatedNuisanceParameters) delete fFluctuatedNuisanceParameters;
     }
     ...
     //void SetDoProfllCheck(Bool_t flag=false){fDoProfllCheck = flag;};
     ...
     virtual Double_t EvaluateProfileLikelihood(int type, RooAbsData &data, RooArgSet & paramsOfInterest);
      ...
      void SetDebuggingData(RooAbsData* uncondData , RooAbsData* condData, RooAbsData* genData , RooAbsData* trueData ) { fDebuggingUncondData = uncondData, fDebuggingCondData = condData, fDebuggingGenData = genData, fDebuggingTrueData = trueData;}

      void SetFluctuatedNuisanceParameters(const RooArgSet set) { fFluctuatedNuisanceParameters = dynamic_cast<RooArgSet*>( set.snapshot() ); }
  ...
  private:
   ...
   private:
      ...
      Double_t fTolerance; 
      Int_t fPrintLevel;
      //Bool_t fDoProfllCheck;
      RooArgSet* fFluctuatedNuisanceParameters;
      RooAbsData* fDebuggingUncondData;
      RooAbsData* fDebuggingCondData;
      RooAbsData* fDebuggingGenData;
      RooAbsData* fDebuggingTrueData;

   protected:
      ClassDef(ProfileLikelihoodTestStat_New,6)   // implements the profile likelihood ratio as a test statistic to be used with several tools
   };
}
